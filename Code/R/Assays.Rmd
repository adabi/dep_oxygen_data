...
plt <- ggplot(data=df_means, mapping = aes(y=response, x=group, fill=treatment), height=10) + 
    ...
    labs(x="", y="LDH (Cell Damage)", fill = "Treatment (μg/ml)", title="LDH Across Groups for SVGp12 - DEP + Ox66") 
...
plot_and_save <- function(cmpnd, assay){
  title <- sprintf("%s for Human Astroglial Cells (SVGp12) - %s", assay, cmpnd)

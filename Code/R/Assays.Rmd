...
df_means <-
  df_scaled %>% 
  filter(compound == "DEP") %>% 
  mutate(treatment = factor(treatment, levels=c("0", "20","50", "100"))) %>% 
...
plt <- ggplot(data=df_means, mapping = aes(y=response, x=group, fill=treatment)) + 
    geom_bar(position="dodge", stat="identity", width=0.5) + 
    geom_errorbar(aes(ymin=y_err_min, ymax=y_err_max), width=.2, position=position_dodge(.5)) + 
    theme_bw(base_size = 11) + 
    labs(x="", y="LDH (Cell Damage)", fill = "Treatment (μg/ml)") 

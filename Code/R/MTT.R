...
file_path= "../../Data/Compiled/SVG_MTT_ALL.csv"
...
plot_bar_graph <- function(cmpnd, multiple.comparisons = FALSE){
  ...
  title <- sprintf("MTT Across Groups for SVGp12 - %s", cmpnd)
  plt <- ggplot(data=df_means, mapping = aes(y=response, x=group, fill=treatment), height=10) + 
    ...
    labs(x="", y="MTT (Cell Damage)", fill = "Treatment (μg/ml)", title=title) 
...
ggsave(p1, filename = "../../Graphs/SVG/MTT//Barplots/DEP.png", device='png', height = 9, width = 16)
ggsave(p2, filename = '../../Graphs/SVG/MTT/Barplots/Ox66.png', device = 'png', height = 9, width = 16)
ggsave(p3, filename = '../../Graphs/SVG/MTT/Barplots/DEP + Ox66.png', device= "png", height = 9, width = 16)

# Fit linear regression and 
plot_and_save <- function(cmpnd, assay){
  title <- sprintf("MTT Across Groups for Human Astroglial Cells (SVGp12) - %s", assay, cmpnd)
  file_path <- sprintf("../../Graphs/SVG/MTT/LinearReg/%s_%s.png", cmpnd, assay)
  df_scaled %>%
    filter(compound==cmpnd) %>%
    mutate(group = factor(group, levels = c("Normoxia - 24 Hours", "Hypoxia - 24 Hours",
                                            "Normoxia - 48 Hours", "Hypoxia - 48 Hours"))) %>% 
    ...
    labs(title= title,
         x="Treatment (μg/ml)", y="MTT (Cell Leakage)", 
...
plot_and_save("DEP", "MTT")
plot_and_save("Ox66", "MTT")
plot_and_save("DEP + Ox66", "MTT")
...
plot_ces <- function(cmpnd, assay){
  ...
  file_name <- sprintf("../../Graphs/SVG/MTT/SlopesCEs/SVG_%s_%s_CE.png", cmpnd, assay)
...
compare_coefs <- function(condition, exposure_time){
  ...
  file_path <- sprintf("../../Graphs/SVG/MTT/SlopesCEs/Compare_%s_%s.png", exposure_time,condition)
  ox66_df <- df_scaled %>% 
